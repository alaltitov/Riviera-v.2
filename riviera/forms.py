...
class LoginForm(FlaskForm):
    ...
    login = StringField(label='Имя пользователя',
                        ...
                        render_kw={"class": "form__login", "placeholder": "Введите имя пользователя"})

    password = PasswordField(label='Пароль',
                             validators=[Length(min=3, max=25, message=f'Пароль должен быть не короче '
                                                                       f'%(min)d и не длиннее %(max)d символов.'),
                                         DataRequired(message='Поле не должно быть пустым')],
                             render_kw={"class": "form__login", "placeholder": "Введите пароль"})

    remember = BooleanField('запомнить', default=False)
    submit = SubmitField('Войти', render_kw={"class": "form__login-btn"})
...
class RegisterForm(FlaskForm):
    ...
    password1 = PasswordField(label='Повторите пароль',
                              ...
                              render_kw={"class": "form__login", "placeholder": "Повторите пароль"})

    email = StringField(label='E-mail', validators=[Email("Некорректный e-mail")],
                        render_kw={"class": "form__login", "id": "email", "placeholder": "Введите e-mail"})

    pin = StringField(label='PIN', render_kw={"class": "form__login", "placeholder": "Введите PIN"})

    tel = StringField(label='Телефон',
                      validators=[Regexp(r'^\+7\(\d{3}\)-\d{3}-\d{2}-\d{2}',
                                         message='Введите номер телефона в формате "+7(999)-999-99-99"')],
                      render_kw={"class": "form__login", "id": "phone", "placeholder": "Введите номер телефона"})

    name = StringField(label='Фамилия, Имя',
                       validators=[DataRequired(message='Поле "Фамилия, Имя" не должно быть пустым')],
                       render_kw={"class": "form__login", "placeholder": "Введите Фамилию и Имя"})

    address = StringField(label='Адрес',
                          validators=[DataRequired(message='Поле "Адрес" не должно быть пустым')],
                          render_kw={"class": "form__login", "placeholder": "Введите адрес"})
...
class SendUtility(FlaskForm):
    ...
    electro = StringField(label='Электроэнергия',
                          validators=[Length(min=1, max=7, message=f'Неверный ввод показаний'),
                                      DataRequired(message='Поле не должно быть пустым')],
                          render_kw={"class": "form__login", "placeholder": "Введите показания счетчика электроэнергии"})

    gas = StringField(label='Газ',
                      validators=[Length(min=1, max=7, message=f'Неверный ввод показаний'),
                                  DataRequired(message='Поле не должно быть пустым')],
                      render_kw={"class": "form__login", "placeholder": "Введите показания счетчика газа"})

    water = StringField(label='Вода',
                        validators=[Length(min=1, max=7, message=f'Неверный ввод показаний'),
                                    DataRequired(message='Поле не должно быть пустым')],
                        render_kw={"class": "form__login", "placeholder": "Введите показания счетчика воды"})

    submit = SubmitField('Отправить', render_kw={"class": "form__login-btn"})
...
class SendPayment(FlaskForm):
    ...
    payment = StringField(label="Сумма к оплате",
                          validators=[DataRequired(message='Поле не должно быть пустым'),
                                      Regexp(r'^[1-9]{1,}(,[0-9]{3})*(.[0-9]{2})+$',
                                             message='Введите сумму с копейками, например "1.00"')],
                          render_kw={"class": "form__payment", "id": "currency"})

    submit = SubmitField('Оплатить', render_kw={"class": "form__payment-btn"})
...
class FeedbackForm(FlaskForm):
    ...
    name = StringField(label='Имя',
                       ...
                       render_kw={"class": "form__feedback", "placeholder": "Введите имя"})

    tel = StringField(label='Телефон',
                      validators=[Regexp(r'^\+7\(\d{3}\)-\d{3}-\d{2}-\d{2}',
                                         message='Введите правильный номер телефона')],
                      render_kw={"class": "form__feedback", "id": "phone_fb", "placeholder": "Введите номер телефона"})
